...
remotes::install_github("rstudio/gradethis", type = "source", dependencies = TRUE)
remotes::install_github("thomasp85/ggforce", type = "source", dependencies = TRUE)
remotes::install_github("hrbrmstr/waffle", type = "source", dependencies = TRUE)
remotes::install_github("cran/poliscidata", type = "source", dependencies = TRUE)
remotes::install_github("cran/flexdashboard", type = "source", dependencies = TRUE)
...
knitr::include_graphics(here::here("www", "img", "aboutpagelook.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img", "exposed.png"))
```
...
knitr::include_graphics(here::here("www", "img", "fernando-cferdo-6x2iKGi6SPU-unsplash.jpg"))
```
...
```{r, eval=FALSE}
# library(OCSdata)
raw_data("ocs-bp-school-shootings-dashboard")
```
...
```{r, echo=FALSE}
shooting_data <- 
  readr::read_csv(file = here::here("www", "data", "raw",
                                    "K-12_SSDB_(Public)-K-12_SSDB_(Public)_Linked.csv"), 
                  col_names = TRUE, skip = 1)
```
...
knitr::include_graphics(here::here("www", "img", "gettodoc.png"))
```

Then we can click on the "share" button to get access to the link:

```{r, echo = FALSE, outwidth = "50%"}
knitr::include_graphics(here::here("www", "img", "gettoshare.png"))
```

Finally we can click on "copy link" button to copy the link:

```{r, echo = FALSE, outwidth = "50%"}
knitr::include_graphics(here::here("www", "img", "getlink.png"))
```
...
```{r, eval = FALSE}
save(shooting_data, file = here::here("www", "data", "imported", "shooting_data.rda"))
```
...
```{r, echo=FALSE}
load(here::here("www", "data", "imported", "shooting_data.rda"))
```
...
```{r, eval=FALSE}
# library(OCSdata)
imported_data("ocs-bp-school-shootings-dashboard")
```
...
```{r, echo = FALSE, outwidth = "50%"}
knitr::include_graphics(here::here("www", "img", "join.png"))
```
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "enable.png"))
```
...
knitr::include_graphics(here::here("www", "img", "shooting_data_coords.png"))
```
...
knitr::include_graphics(here::here("www", "img", "coords.png"))
```
...
knitr::include_graphics(here::here("www", "img", "double.png"))
```
...
save(shooting_data_wrangled_pre_map, file = here::here("www", "data", "wrangled",
                             "shooting_data_wrangled_pre_map.rda"))

readr::write_csv(shooting_data_wrangled_pre_map, 
                 file = here::here("www", "data", "wrangled",
...
```{r, eval=FALSE}
load(file = here::here("data", "wrangled",
                             "shooting_data_wrangled_pre_map.rda"))
```

```{r, echo=FALSE}
load(file = here::here("www", "data", "wrangled",
                             "shooting_data_wrangled_pre_map.rda"))
```
...
```{r}
shooting_data_wrangled_pre_map[1:4]
```
...
```{r, echo = FALSE, out.width= "90%"}
knitr::include_graphics(here::here("www", "img", "geometry.png"))
```
...
```{r exploration_wrangling_quiz, echo = FALSE}
quiz(caption = "",
  question("Which function from `dplyr` should be used to combine two data frames `x` and `y` while preserving all rows and columns from both?",
    answer("`inner_join(x,y)`", message = "This will preserve only the rows that have matches between `x` and `y`."),
    answer("`left_join(x,y)`", message = "This will not preserve all the rows from y."),
    answer("`right_join(x,y)`", message = "This will not preserve all the rows from x."),
    answer("`full_join(x,y)`", correct = TRUE),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  question("Which package is best for formatting calendar date values?",
    answer("`dplyr`", message = "This package is more suited for handling whole data frames. This data may consist of calendar dates, however."),
    answer("`ggplot2`", message = "This package is used for visualizing data."),
    answer("`ggmap`", message = "This package is used for geocoding maps."),
    answer("`sf`", message = "This package is used for creating geometry lists or simple feature objects."),
    answer("`lubridate`", correct = TRUE),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  question("Which function from `dplyr` do we use to extract columns of interest from a data frame?",
    answer("`filter()`", message = "This will extract rows of interest."),
    answer("`select()`", correct = TRUE),
    answer("`distinct()`", message = "This removes duplicate rows."),
    answer("`slice()`", message = "This will extract rows by position."),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
)
...
Write some code that will change the `vs` and `am` columns in `mtcars` from numeric to logical values. Save this as a new object called `mtcars_log`. 
...
```{r dew_code1, exercise=TRUE}
mtcars_log = ______ %>%
  ______(__ = dplyr::case_when(__ == 0 ~ _____,  # step 1
                               __ == 1 ~ ____), 
         __ = dplyr::case_when(__ == 0 ~ _____,  # step 2
                               __ == 1 ~ ____))
...
```{r dew_code1-hint-2}
mtcars_log = mtcars %>%
  mutate(vs = dplyr::case_when(vs == 0 ~ FALSE,  # step 1
                               vs == 1 ~ TRUE), 
         __ = dplyr::case_when(__ == 0 ~ _____,  # step 2
                               __ == 1 ~ ____))
...
```{r dew_code1-solution}
mtcars_log = mtcars %>%
  mutate(vs = dplyr::case_when(vs == 0 ~ FALSE,  # step 1
                               vs == 1 ~ TRUE), 
         am = dplyr::case_when(am == 0 ~ FALSE,  # step 2
                               am == 1 ~ TRUE))
...
<!-- chunk causing issues with learnr and gradethis renderings: -->
...
<!-- embedd a link to the DT table in a separate repository -->


<iframe src="https://www.opencasestudies.org/school-shootings-datatable/" height="720" width="1080" style="border: 1px solid #464646;" allowfullscreen="" allow="autoplay" data-external="1"></iframe>
...
```{r, echo = FALSE, outwidth = "40%"}
knitr::include_graphics(here::here("www", "img", "bindrows.png"))
```
...
```{r analysis_visualization_quiz, echo = FALSE}
quiz(caption = "",
  question("Which function from `ggplot2` do we use to create a bar plot of raw values instead of counts?",
    answer("`geom_line()`", message = "This creates a continuous line plot."),
    answer("`geom_point()`", message = "This creates a scatter plot."),
    answer("`geom_boxplot()`", message = "This creates a box plot."),
    answer("`geom_bar()`", message = "Close, however this function defaults to using counts rather than the raw values themselves."),
    answer("`geom_col()`", correct = TRUE),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  question("Which package do we use to create searchable tables?",
    answer("`dplyr`", message = "This package is used for wrangling data frames."),
    answer("`ggplot2`", message = "This package is used for visualizing data."),
    answer("`ggmap`", message = "This package is used for geocoding maps."),
    answer("`sf`", message = "This package is used for creating geometry lists or simple feature objects."),
    answer("`DT`", correct = TRUE),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  question("What does the function `pivot_longer()` do?",
    answer("It increases the number of columns or variables and decreases rows or observations.", message = "This describes the `pivot_wider()` function."),
    answer("It decreases the number of columns or variables and increases rows or observations.", correct = TRUE),
    answer("It increases both the number of columns or variables and rows or observations.", message = "This would require collecting more data!"),
    answer("It does nothing!", message = "It does, in fact, do something."),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
)
...
Write some code that will create a bar plot of the raw values in the `mpg` column for each car in `mtcars` using `ggplot2` and `geom_col()`.
...
```{r dav_code2, exercise=TRUE}
mpg_plot = ______(______, aes(x = row.names(mtcars), y = ___)) + ________() + 
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) 
  # this theme command will make the x-axis labels easier to read
  # this part is provided for you since it is not covered in the case study.
...
```{r dav_code2-hint-1}
mpg_plot = ggplot(mtcars, aes(x = row.names(mtcars), y = ___)) + ________() + 
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) 
  # this theme command will make the x-axis labels easier to read
  # this part is provided for you since it is not covered in the case study.
...
```{r dav_code2-hint-2}
mpg_plot = ggplot(mtcars, aes(x = row.names(mtcars), y = mpg)) + ________() + 
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) 
  # this theme command will make the x-axis labels easier to read
  # this part is provided for you since it is not covered in the case study.
...
```{r dav_code2-solution}
mpg_plot = ggplot(mtcars, aes(x = row.names(mtcars), y = mpg)) + geom_col() + 
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5))
  # this theme command will make the x-axis labels easier to read
  # this part is provided for you since it is not covered in the case study.
...
```{r, echo = FALSE, out.width= "60%"}
knitr::include_graphics(here::here("www", "img", "game.png"))
```
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "syntax.png"))
```
...
Like so:
```{r, echo = FALSE, out.width = "30%"}
knitr::include_graphics(here::here("www", "img", "yaml.png"))
```
...
```{r, echo = FALSE, out.width = "30%"}
knitr::include_graphics(here::here("www", "img", "yaml_dashboard.png"))
```
...
3. To add columns or rows the following syntax is used `---------`. By default this notation will create new columns, however if the YAML is modified to specify to create rows, then this same syntax will be used to create rows. The number of dashes does not matter. (This is a level 2 header in Markdown, just like `##`)
...
```{r, echo = FALSE, out.width = "30%"}
knitr::include_graphics(here::here("www", "img", "code_chunk.png"))
```
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "gauge.png"))
```
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "columns.png"))
```
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "rows.png"))
```
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "tab.png"))
```
...
```{r, echo = FALSE, out.width = "30%"}
knitr::include_graphics(here::here("www", "img", "shiny_yml.png"))
```
...
knitr::include_graphics(here::here("www", "img", "knit.png"))
```

Then you can host this on GitHub if you choose by changing the GitHub Pages settings of your repository:

```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "githubpages.png"))
```
...
knitr::include_graphics(here::here("www", "img", "publish.png"))
```
...
```{r, echo = FALSE, fig.link = "https://rsconnect.biostat.jhsph.edu/ocs-bp-school-shootings-dashboard/"}
knitr::include_graphics(here::here("www", "img", "dashboard_school.png"))
```
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "newrmd.png"))
```
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "our_yaml.png"))
```
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "readable.png"))
```
...
```{r, eval = FALSE}
shooting_data <- 
  read_csv(here::here("www", "data", "wrangled",
                "shooting_data_wrangled_pre_map.csv"))
...
```{r, echo = FALSE, out.width="60%"}
knitr::include_graphics(here::here("www", "img", "fontawesome.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img" , "about_page_structure.png"))
```
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "markdownsyntax.png"))
```
...
```{r, echo=FALSE, out.width="60%"}
knitr::include_graphics(here::here("www", "img" , "project.png"))
```
...
```{r, echo=FALSE, out.width="60%"}
knitr::include_graphics(here::here("www", "img" , "newdir.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img" , "The_Data_Page.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img" , "Data_page_structure.png"))
```
...
```{r, echo = FALSE, out.width="60%"}
knitr::include_graphics(here::here("www", "img", "tags.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img" , "US_Statistics_page.png"))
```
...
```{r, echo=FALSE, out.width="90%"}
knitr::include_graphics(here::here("www", "img" , "US_stats_page_overview.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img" , "State_Statistics_page.png"))
```
...
```{r, echo=FALSE, out.width="90%"}
knitr::include_graphics(here::here("www", "img" , "states_stats_page_overview.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img" , "The_Map_Page.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img" , "map_page_structure.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img" , "the_Tutorial_Page.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img" , "tutorialStructure.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img" , "The_Get_Help_Page.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img" , "The_Get_Help_Page_structure.png"))
```
